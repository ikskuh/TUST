...
bool VoronoiDiagramGenerator::generateVoronoi(std::vector<float> *xValues, std::vector<float> *yValues, float minX, float maxX, float minY, float maxY, float minDist)
{
	cleanup();
	cleanupEdges();
	int i;
	...
	for(i = 0; i< nsites; i++)
	{
		sites[i].coord.x = xValues->at(i);
		sites[i].coord.y = yValues->at(i);
		sites[i].sitenbr = i;
		sites[i].refcnt = 0;

		if(xValues->at(i) < xmin)
			xmin = xValues->at(i);
		else if(xValues->at(i) > xmax)
			xmax = xValues->at(i);

		if(yValues->at(i) < ymin)
			ymin = yValues->at(i);
		else if(yValues->at(i) > ymax)
			ymax = yValues->at(i);
